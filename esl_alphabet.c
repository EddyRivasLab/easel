 ...
 *****************************************************************/
static void
dump_symbol_residue_table(FILE *fp, const ESL_ALPHABET *a)
{
  fprintf(fp, "Symbol-to-residue mapping:\n");

  /* Table header. */
  fprintf(fp, "");
  int res_word_len = strlen("residue");
  int sym_word_len = strlen("symbol");
  int res_01_len = a->K * 2 - 1;
  int rcol_len = ESL_MAX(res_01_len, res_word_len);
  fprintf(fp, "  | Symbol | Residue%*s |\n", ESL_MAX(0, rcol_len - res_word_len), "");

  /* Table header-to-body separation. */
  fprintf(fp, "  | ------ | ");
  for (int i = 0; i < rcol_len; ++i) fputc('-', fp);
  fprintf(fp, " |\n");

  /* Table body. */
  for (int i = 0; i < a->Kp; i++)
  {
    fprintf(fp, "  | %*s %c |", sym_word_len - 2, "", a->sym[i]);
    for (int j = 0; j < a->K; j++)
    {
      fprintf(fp, " %d", (int) a->degen[i][j]);
    }
    fprintf(fp, " |\n");
  }
}
...
static void
dump_symbol_complement(FILE *fp, const ESL_ALPHABET *a)
{
  fprintf(fp, "Complement mapping:\n");
  for (int i = 0; i < a->Kp; ++i)
    fprintf(fp, "  %c->%c\n", a->sym[i], a->sym[a->complement[i]]);
}

static void
dump_inmap(FILE *fp, const ESL_ALPHABET *a)
{
  fprintf(fp, "Index mapping:\n");
  for (int i = 0; i < 128; ++i)
  {
    if (a->inmap[i] != eslDSQ_ILLEGAL && a->inmap[i] != eslDSQ_IGNORED)
      fprintf(fp, "  %c->%d\n", (char) i, a->inmap[i]);
  }
}


/* Function:  esl_alphabet_Dump()
 * Synopsis:  Dump alphabet sizes, symbols, and mappings.
 *
 * Purpose:   Write a summary of the alphabet structure.
 * Args:      fp - file pointer to direct the output.
 *            a  - digital alphabet to use.
 *
 * Returns:   <eslOK> if it runs successfully; <eslFAIL> if not.
 */
int
esl_alphabet_Dump(FILE *fp, const ESL_ALPHABET *a)
{
  const char *dt = esl_abc_DecodeType(a->type);
  if (dt == NULL) return eslFAIL;

  fprintf(fp, "Alphabet type: %s\n", dt);
  fprintf(fp, "Unique alphabet size: %d\n", a->K);
  fprintf(fp, "Total alphabet size: %d\n", a->Kp);
  fprintf(fp, "Symbols: %s\n", a->sym);

  dump_symbol_residue_table(fp, a);
  if (a->type == eslRNA || a->type == eslDNA) dump_symbol_complement(fp, a);
  dump_inmap(fp, a);

  return eslOK;
}
/*---------------- end, debugging/development utilities ---------*/
...
utest_Dump()
{
  char         *msg = "Dump unit test failure";
  ESL_ALPHABET *a;

  a = esl_alphabet_Create(eslAMINO);
  if (esl_alphabet_Dump(stdout, a) != eslOK) esl_fatal(msg);
  esl_alphabet_Destroy(a);
  ...
  a = esl_alphabet_CreateCustom("ACDEFGHIKLMNPQRSTVWY-BJZOUX~", 20, 28);
  if (a == NULL) esl_fatal(msg);
  if (esl_alphabet_SetIgnored(a, " \t") != eslOK) esl_fatal(msg);
  if (esl_alphabet_SetCaseInsensitive(a) != eslOK) esl_fatal(msg);
  if (esl_alphabet_SetDegeneracy(a, 'Z', "QE") != eslOK) esl_fatal(msg);
