...
of Easel's \eslmod{evd} module, where the Gumbel distribution is the
...
The \eslmod{evd} API consists of the following functions:
...
obtained by a call to \ccode{esl\_evd\_Pgeqx()}.  The E-value for
...
The \ccode{esl\_evd\_Sample()} function requires augmenting with the
\eslmod{random} module.

The \ccode{esl\_evd\_FitTruncated()} function requires augmenting with
