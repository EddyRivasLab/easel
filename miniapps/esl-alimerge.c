...
main(int argc, char **argv)
{
  ...
  if (! esl_opt_IsDefault(go, "--outformat")) {
    outfmt = esl_sqio_FormatCode(esl_opt_GetString(go, "--outformat"));
 ...
 * annotation: ss_cons, sa_cons, and rf. With rf,
...
validate_and_copy_msa_annotation(const ESL_GETOPTS *go, int outfmt, ESL_MSA *mmsa, ESL_MSA **msaA, int nmsa, int clen, int alen_merged, int *maxinsert, char *errbuf)
{
   ...
   * of the first child msa, so we can (possibly) gap out GC,SS_cons,SA_cons annotation 
  ...
  if(msaA[0]->au != NULL) { 
    for(ai = 1; ai < nmsa; ai++) { 
      if((msaA[ai]->au == NULL) || (strcmp(msaA[0]->au, msaA[ai]->au) != 0)) { do_add = FALSE; break; }
    }
    if(do_add) { 
  ...
  if(msaA[0]->ngf > 0) { 
	...
	for(j2 = 0; j2 < msaA[ai]->ngf; j2++) { 
	  if(strcmp(msaA[0]->gf_tag[j], msaA[ai]->gf_tag[j2]) == 0) {
	    found_tag = TRUE;
	    if(strcmp(msaA[0]->gf[j], msaA[ai]->gf_tag[j2]) != 0) { 
	      do_add = FALSE; 
	    }
  ...
  if(msaA[0]->ncomment > 0) { 
	...
	for(ai = 1; ai < nmsa; ai++) { 
	  if(strcmp(msaA[0]->comment[j], msaA[ai]->comment[j]) != 0) { /* comment doesn't match */
	    do_add = FALSE;
	    break;
	  }
	}
  ...
  if(msaA[0]->ngc > 0) { 
	...
	for(ai = 1; ai < nmsa; ai++) { 
	  ...
	  for(j2 = 0; j2 < msaA[ai]->ngc; j2++) { 
	    if(strcmp(msaA[0]->gc_tag[j], msaA[ai]->gc_tag[j2]) == 0) {
	      found_tag = TRUE;
	      /* ensure it does not have non-gaps in gap RF columns */
		...
		/* check identity */
		if(strcmp(dealigned, dealigned2) != 0) { do_add = FALSE; }
		free(dealigned2); 
		dealigned2 = NULL; 
  ...
  for(ai = 1; ai < nmsa; ai++) { 
    ...
    /* check identity */
    if(strcmp(dealigned, dealigned2) != 0) { 
