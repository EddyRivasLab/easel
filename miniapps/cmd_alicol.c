 ...
 *
 * Currently requires one of two options:
...
static ESL_OPTIONS cmd_options[] = {
  ...
  { "--mingap",   eslARG_NONE,   FALSE, NULL, NULL, NULL, NULL,       "--nogap",   "remove columns that contain only gaps",                           0 },
  { "--keeprf",   eslARG_NONE,   FALSE, NULL, NULL, NULL, "--mingap", NULL,        "with --mingap, keep all consensus columns (nongaps in #=GC RF)",  0 },
  { "--nogap",    eslARG_NONE,   FALSE, NULL, NULL, NULL, NULL,       "--mingap",  "remove columns containing any gaps",                              0 },
  ...
  while ((status = esl_msafile_Read(afp, &msa)) != eslEOF)
    {
      ...
      if (status != eslOK) esl_fatal("alicol column filtering failed:\n%s", errbuf);
