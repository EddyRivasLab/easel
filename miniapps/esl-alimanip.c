...
main(int argc, char **argv)
{
  ...
  while ((status = esl_msa_Read(afp, &msa)) == eslOK)
	...
	ESL_MSA *new_msa;
	msa_remove_seqs_above_minlen(msa, maxlen, &new_msa);
 ...
 ERROR:
  ...
  return 0.; /* NEVERREACHED */
	...
	write_ali = TRUE;
      ...
      /* handle the --tambig option if enabled, remove any trailing/leading ambiguous characters */
      if( esl_opt_IsOn(go, "--tmabig")) {
	ESL_MSA *new_msa;
	if((status =msa_remove_truncated_seqs(msa, errbuf, esl_opt_GetInteger(go, "--detrunc"), &new_msa)) != eslOK) esl_fatal(errbuf);
	/* new_msa is msa without seqs below minlen, swap ptrs */
	esl_msa_Destroy(msa);
