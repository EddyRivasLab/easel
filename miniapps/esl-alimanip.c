...
static ESL_OPTIONS options[] = {
  ...
  { "--lnfract",   eslARG_REAL,  NULL,  NULL, "0<=x<=2", NULL,NULL, NULL,                       "remove sequences w/length < <x> fraction of median length",      4 },
  { "--lxfract",   eslARG_REAL,  NULL,  NULL, "0<=x<=3", NULL,NULL, NULL,                       "remove sequences w/length > <x> fraction of median length",      4 },
  { "--lmin",      eslARG_INT,   NULL,  NULL, "n>0",     NULL,NULL, NULL,                       "remove sequences w/length < <n> residues",                       4 },
  { "--lmax",      eslARG_INT,   NULL,  NULL, "n>0",     NULL,NULL, NULL,                       "remove sequences w/length > <n> residues",                       4 },
  ...
  while ((status = esl_msa_Read(afp, &msa)) == eslOK)
    {
      ...
      /* first handle the --lnfract and/or --lxfract options if enabled, all subsequent manipulations will omit any seqs removed here */
      if (esl_opt_IsOn(go, "--lnfract")) {
	median   = msa_median_length(msa);
	float minlen = esl_opt_GetReal(go, "--lnfract") * (float) median;
	ESL_MSA *new_msa;
	...
	median   = msa_median_length(msa);
	float maxlen = esl_opt_GetReal(go, "--lxfract") * (float) median;
	ESL_MSA *new_msa;
	...
	write_ali = TRUE;
      ...
      /* handle the --lmax option if enabled, all subsequent manipulations will omit any seqs removed here */
      if (esl_opt_IsOn(go, "--lmax")) {
	float maxlen = esl_opt_GetInteger(go, "--lmax");
	ESL_MSA *new_msa;
	msa_remove_seqs_above_maxlen(msa, maxlen, &new_msa);
...
/* handle_post_opts
 *                   
 * Read "#=GR PP" or "#=GR POST" annotation into a 2D matrix, each 
 * sequence is a row, each residue is a column. Handle any command line
...
static int handle_post_opts(const ESL_GETOPTS *go, char *errbuf, ESL_MSA *msa)
{
  ...
  int have_pp = FALSE;   /* TRUE if msa has #=GR PP annotation, we'll use this if it exists, else we look for #=GR POST annotation generated by infernal 0.72-1.0 */
  ...
  if(msa->pp != NULL) { /* MSA has GR PP annotation, use that */
    have_pp = TRUE; 
  }
  else { 
    ...
    if(ndigits == 0) { 
      if(do_pfract) ESL_FAIL(eslEINVAL, errbuf, "--pfract requires \"#=GR PP\", \"#=GR POST\", \"#=GR Post\", \"#=GR post\", \"#=GR POSTX.\", or \"#=GR POSTX.\" and \"#=GR POST.X\" annotation in %s.\n", esl_opt_GetArg(go,1));
      if(do_pinfo)  ESL_FAIL(eslEINVAL, errbuf, "--pinfo  requires \"#=GR PP\", \"#=GR POST\", \"#=GR Post\", \"#=GR post\", \"#=GR POSTX.\", or \"#=GR POSTX.\" and \"#=GR POST.X\" annotation in %s.\n", esl_opt_GetArg(go,1));
    }
    ...
    if(ndigits == 2 && (ridx1 == -1 || ridx2 == -1)) { 
      if(do_pfract) ESL_FAIL(eslEINVAL, errbuf, "--pfract requires \"#=GR PP\", \"#=GR POST\", \"#=GR Post\", \"#=GR post\", or \"#=GR POSTX.\" and \"#=GR POST.X\" annotation in %s.\n", esl_opt_GetArg(go,1));
      if(do_pinfo)  ESL_FAIL(eslEINVAL, errbuf, "--pinfo  requires \"#=GR PP\", \"#=GR POST\", \"#=GR Post\", \"#=GR post\", or \"#=GR POSTX.\" and \"#=GR POST.X\" annotation in %s.\n", esl_opt_GetArg(go,1));
    }
    if(msa->rf == NULL) { 
  ...
  /* if we get here, either have_pp==TRUE and we have PP annotation or have_pp==FALSE and we have GR POST annotation, either way we can deal */
  ...
  else { /* msa does not have #=GR PP annotation, but it does have infernal GR POST annotation, which we can handle */
    if(ndigits == 1) {    
      for(s = 0; s < msa->nseq; s++) { 
	for(c = 0; c < msa->alen; c++) { 
