...
system\footnote{You need a C compilation environment, which may not be
installed. If not, install the free GNU C compiler, or another compiler, or
see your local systems guru if you can't compile C programs.} with
this incantation (where \ccode{xxx} is a version number):
...
or more \textbf{driver programs}, including programs for unit tests
...
always at least one \textbf{example} driver that shows you how to use
the module. You can find the example code in module \eslmod{foo} by
...
example, you're on your way. The next section (below) will introduce
the different modules in Easel, so you can see what you might be
interested in. When you want to know more about a module, either check
that module's chapter in this guide, or just read the module's source
code. Most of the documentation is compiled automatically from the
source code comments anyway.
