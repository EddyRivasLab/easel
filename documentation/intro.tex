...
database and the Rfam RNA family database. Easel is meant to maintain
a body of work from my lab in a useful form, such that new people in
the lab can avoid reinventing the wheel as many times as I have.

Easel has arisen by accretion. As such, it is not intended to be an
all-purpose sequence analysis library.  General-purpose bioinformatics
libraries are available that do aspire to such goals. The NCBI Toolkit
and the Ajax library in EMBOSS are comprehensive resources for C
programming. The BioPerl project is an extraordinary resource for
Perl.

Easel's design reflects some of my opinions about the way the world
should work but usually doesn't. First, it is \textbf{documented}.
And not just by mass of paper -- there are tutorial examples to every
Easel API, intended to make it easy to start using any given Easel
module.
