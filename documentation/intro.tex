...
sequences using probabilistic models.  Easel is used by \prog{HMMER}
\citep{hmmer,Eddy98}, the profile hidden Markov model software that
underlies the \prog{Pfam} protein family database
\citep{Finn06,Sonnhammer97} and several other protein family
databases. Easel is also used by \prog{Infernal}
\citep{infernal,NawrockiEddy07}, the covariance model software that
underlies the \prog{Rfam} RNA family database
...
your local system guru.} with this incantation (where \ccode{xxx} is a
version number):
