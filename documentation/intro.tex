...
\footnote{The current poster child of test code is the \eslmod{msa}
module, as of 8 Sept 2006, but even it is not fully compliant; it does
not exercise all augmentations yet.}

\begin{enumerate}

\item Each module shall have a test driver \ccode{main()}, compiled by 
      a \ccode{-DeslFOO\_TESTDRIVE} flag.

\item The testdriver program must return \ccode{eslOK} (zero, a normal
...
\begin{cchunk}
      % gcc -g -Wall -o test -I. -DeslMSA_TESTDRIVE esl_msa.c easel.c
      % gcc -g -Wall -o test -I. -DeslMSA_TESTDRIVE -DeslAUGMENT_ALPHABET esl_msa.c esl_alphabet.c easel.c
      % gcc -g -Wall -o test -I. -L. -DeslMSA_TESTDRIVE esl_msa.c -leasel -lm
\end{cchunk}

\item The testdriver shall respond to a \ccode{eslTEST\_THROWING}
...
\item Upon detecting a failure, the testdriver shall call
      \ccode{abort()} as soon as possible to facilitate
      tracing/debugging (or equivalently, it uses
      \ccode{assert()}). Garbage collection (including memory, opened
      files, or created temporary files) is not required, and may be
      counterproductive -- it may be advantageous for debugging
      purposes to leave the temporary file for a failed test on disk,
      for debugging.

\item Each function, procedure, and macro in the exposed API shall
      have a unit test procedure. A unit test for
      \ccode{esl\_foo\_Baz()} is named (\ccode{static int
      utest\_Baz()}. A unit test returns \ccode{eslOK} on success.  On
      detecting a failure, a unit test should call \ccode{abort()}
      immediately to facilitate tracing/debugging; less preferably, a
      unit test may return \ccode{eslFAIL} to the main test driver.
