...
\ccode{esl\_fatal()} with an informative error message, or by simply
calling \ccode{abort()}. Garbage collection (including memory, opened
...
an \ccode{abort()} call.  The automated \ccode{sqc} test harness
...
and a failure that calls \ccode{abort()} like:
...
and a failure that calls \ccode{esl\_fatal()} like:
