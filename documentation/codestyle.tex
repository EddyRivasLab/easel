...
\ccode{\_GrowTo}
  & Reallocate object if necessary for some new data size.
  & [standard]
...
Some resizeable objects take a \ccode{nredline} parameter that
controls an additional memory management rule. These objects are
allowed to \ccode{_GrowTo()} arbitrary size, but when they exceed an
allocation size of \ccode{nredline}, they shrink back to the
\ccode{nredline} limit upon the next call to \ccode{_Reuse()}.  The
idea is to be able to briefly handle rare extremely large problems in
memory, without permanently committing the resizeable object to the
most extreme size. 
...
\ccode{esl\_dmatrix\_Compare()} as a test for successful convergence of
...
is in progress (such as HMMER's \ccode{p7\_DEBUGGING} flag).
