...
in its style. The purpose of describing Easel's style is not to
enforce foolish consistency. Still, one way we try to manage the
complexity of our large codebase is to cajole Easel code into a clean
and consistent presentation. We try to organize code modules in
...
Easel module's \ccode{.c} file is the actual module implementation.
Typically, well more than half of an Easel \ccode{.c} file is
...
date) to help track the provenance of this file \footnote{A nostalgic
  ...
  listening to may also be noted on the incept line.}; and \ccode{SVN
  \$Id\$} and \ccode{SVN \$URL\$} tags that are filled in by
...
Like \ccode{\_Set}, but with \ccode{printf()}-style semantics.  Sets
some string value in \ccode{obj} according to the
\ccode{printf()}-style \ccode{fmtstring} and any subsequence
\ccode{printf()}-style arguments. If a value was already set, it is
...
\ccode{\#define} or \ccode{enum} code. For example,
...
thrown error) because we will often use an \ccode{Encode} function to
...
\ccode{-fprofile-arcs -ftest-coverage}. An example:

\begin{cchunk}
  % make distclean
  % ./configure --enable-debugging
  % make
  % gcc -g -Wall -fprofile-arcs -ftest-coverage -I. -L. -o esl_buffer_utest -DeslBUFFER_TESTDRIVE esl_buffer.c -leasel -lm
  % ./esl_buffer_utest
  % gcov esl_buffer.c
  File 'esl_buffer.c'
