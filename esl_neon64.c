 ...
 */
esl_neon_128f_t esl_neon64_logf(esl_neon_128f_t x) {
  ...
  */
  ...
  x = esl_neon64_select_float(x, neginf_vector, poszero_mask_view); /* mask +0 */
  x = esl_neon64_select_float(x, inf_vector, inf_mask_view); /* mask +inf */ 
  return x;
}

/* Function:  esl_neon64_expf()
 ...
 */
esl_neon_128f_t 
esl_neon64_expf(esl_neon_128f_t x) 
{
  ...
  y = esl_neon64_select_float(y, inf_vec, maxmask_view); /* exp(x) = inf for x > log(2^128) */
  y = esl_neon64_select_float(y, zero_vec, minmask_view); /* exp(x) = 0 for x < log(2^-149) */
  return y; 
}
...
void
esl_neon64_dump_float(FILE *fp, esl_neon_128f_t v)
{
  float *p = (float *)&v;
...
/* gcc -mfpu=neon -O3 -o benchmark-neon -I ~/src/hmmer/easel -L ~/src/hmmer/easel -DeslNEON_BENCHMARK esl_neon64.c -leasel -lm
 */
#include "esl_config.h"
...
main(int argc, char **argv)
{
  ...
  /* Vector time */
  esl_stopwatch_Start(w);
  for (i = 0; i < N; i++) { xv = esl_neon64_logf(xv); xv = esl_neon64_expf(xv); }
  esl_stopwatch_Stop(w);
  ...
  esl_neon64_dump_float(stdout, xv); printf("\n");
...
utest_logf(ESL_GETOPTS *go)
{
  ...
  r.v =  esl_neon64_logf(x); 
  if (esl_opt_GetBoolean(go, "-v")) {
    printf("logf");
    esl_neon64_dump_float(stdout, x);    printf(" ==> ");
    esl_neon64_dump_float(stdout, r.v);  printf("\n");
  }
  ...
  for (i = 0; i < N; i++)
    {
      ...
      r1.v      = esl_neon64_logf(tmp);  /* r1.x[z] = log(p1/p2) */
      scalar_r1 = log(odds);
      ...
      r2.v      = esl_neon64_expf(r1.v);        /* and back to odds */
...
#ifdef eslNEON_TESTDRIVE
/* gcc -mfpu=neon -g -Wall -o test -I. -L. -DeslNEON_TESTDRIVE esl_neon64.c -leasel -lm
 */
#include "esl_config.h"
...
/* gcc -mfpu=neon -g -Wall -o example -I. -L. -DeslNEON_EXAMPLE esl_neon64.c -leasel -lm
 */
#include "esl_config.h"
...
main(int argc, char **argv)
{
  ...
  rv.v = esl_neon64_logf(xv);
  printf("logf(%f) = %f\n", x, rv.x[0]);
  
  rv.v = esl_neon64_expf(xv);
  printf("expf(%f) = %f\n", x, rv.x[0]);
...
void esl_neon64_DoAbsolutelyNothing(void) { return; }
#if defined eslNEON_TESTDRIVE || eslNEON_EXAMPLE || eslNEON_BENCHMARK
int main(void) { return 0; }
#endif

#endif /* HAVE_NEON64 or not*/


/* esl_neon64_logf() and esl_neon64_expf() are additionally:
