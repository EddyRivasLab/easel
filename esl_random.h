...
extern void            esl_randomness_Destroy(ESL_RANDOMNESS *rng);
extern int             esl_randomness_Init(ESL_RANDOMNESS *rng, uint32_t seed);
extern uint32_t        esl_randomness_GetSeed(const ESL_RANDOMNESS *rng);

/* 2. The generator, esl_random().
 */
extern double   esl_random       (ESL_RANDOMNESS *rng);
extern uint32_t esl_random_uint32(ESL_RANDOMNESS *rng);
extern uint64_t esl_random_uint64(ESL_RANDOMNESS *rng);
extern int      esl_rnd_Roll     (ESL_RANDOMNESS *rng, int n);
extern int64_t  esl_rnd_LRoll    (ESL_RANDOMNESS *rng, int64_t n);
 ...
 */
extern double esl_rnd_UniformPositive(ESL_RANDOMNESS *rng);
