...
struct esl_alphabet_s {
  ...
  char **degen;          /* 1/0 for which syms include which residues     [0..Kp-1][0..K-1] */
...
extern int esl_abc_CreateDigitalSequence(ESL_ALPHABET *a, char *seq, int L, char **ret_dsq);
extern int esl_abc_DigitizeSequence(ESL_ALPHABET *a, char *seq, int L, char *dsq);
...
extern float  esl_abc_ExpectFScore(ESL_ALPHABET *a, char x, float *sc, float *p);
extern double esl_abc_ExpectDScore(ESL_ALPHABET *a, char x, double *sc, double *p);
