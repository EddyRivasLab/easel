 ...
 *****************************************************************************/
static int write_afa      (FILE *fp, const ESL_MSA *msa);
 ...
 */
static int
write_afa(FILE *fp, const ESL_MSA *msa)
{
  int     i;
  int64_t pos;
  char    buf[61];
...
read_afa(ESL_MSAFILE *afp, ESL_MSA **ret_msa)
{
  ...
  while ((status2 = msafile_getline(afp)) == eslOK) 
	...
	seqidx = msa->nseq;
	msa->nseq++;
	if (esl_strtok(&s, " \t\n\r", &seqname, &len) != eslOK) ESL_XFAIL(eslEFORMAT, afp->errbuf, "AFA MSA parse error, problem reading name of sequence %d at line %d\n", seqidx+1, afp->linenumber);
	status = esl_strdup(seqname, len, &(msa->sqname[seqidx]));

	/* add description, if it exists */
	status = esl_strtok(&s, "\n\r", &desc, &len);
	if(status == eslOK) { /* add description */
	  if (msa->sqdesc == NULL) { /* doesn't exist yet, allocate it */
	    ESL_ALLOC(msa->sqdesc, sizeof(char *) * msa->sqalloc); 
	    for (i = 0; i < msa->sqalloc; i++) msa->sqdesc[i] = NULL;
	  } 
	  if((status = esl_strdup(desc, len, &(msa->sqdesc[seqidx]))) != eslOK) goto ERROR;
	}
