...
esl_alphabet_CreateCustom(char *alphabet, int K, int Kp)
{
  ...
  /* Allocation.
   */
  if ((a = malloc(sizeof(ESL_ALPHABET))) == NULL) goto FAILURE1;
  a->sym    = NULL;
  a->degen  = NULL;
  a->ndegen = NULL;
  
  if ((a->sym    = malloc(sizeof(char)   * (Kp+1))) == NULL) goto FAILURE2;
  if ((a->degen  = malloc(sizeof(char *) * Kp))     == NULL) goto FAILURE2;
  if ((a->ndegen = malloc(sizeof(int)    * Kp))     == NULL) goto FAILURE2;
  a->degen[0] = NULL;

  if ((a->degen[0] = malloc(sizeof(char) * (Kp*K))) == NULL) goto FAILURE3;
  for (x = 1; x < Kp; x++)
...
esl_alphabet_SetSynonym(ESL_ALPHABET *a, char sym, char c)
{
  ...
  if ((sp = strchr(a->sym, c)) == NULL) ESL_ERROR(ESL_EINVAL, "symbol not in the alphabet");
  x = sp - a->sym;
  a->inmap[(int) sym] = x;
  return ESL_OK;
}
...
esl_alphabet_SetDegeneracy(ESL_ALPHABET *a, char c, char *ds)
{
  ...
  if ((sp = strchr(a->sym, c)) == NULL) ESL_ERROR(ESL_EINVAL, "no such degenerate character");
  x = sp - a->sym;
  
  while (*ds != '\0') {
    if ((sp = strchr(a->sym, *ds)) == NULL) ESL_ERROR(ESL_EINVAL, "no such base character");
    y = sp - a->sym;
 ...
 *
 * Purpose:   Given an allocated <dsq> of length <L> (that is, array of <L>+2
 *            chars), reuse it, digitizing up to <L> characters
 *            of <seq>, according to the alphabet <a>. <seq> may be of any length,
 *            but <dsq> will not contain more than <L> characters of it. 
 *            
 *            Usually, both <dsq> and <seq> have the same number of residues <L>, 
 *            but the API allows <dsq> to be a window on a longer sequence.
 *            
 * Args:      a      - internal alphabet
 *            seq    - alphabetic input sequence
 *            L      - allocated length of dsq; maximum length of <seq> to digitize.
 *            dsq    - allocated space for digital sequence, 1..L
 *
 * Returns:   <ESL_OK> on success, and <dsq> contains the newly digitized <seq>.
...
/*****************************************************************
 * Test driver:
 *   gcc -g -Wall -I. -o alphabet_test -DESL_ALPHABET_TESTDRIVE alphabet.c easel.c 
 *****************************************************************
 */ 
...
basic_examples(void)
{
  ...
  esl_alphabet_SetSynonym(a2, 'U', 'S');     /* read selenocysteine U as a serine S */
...
degeneracy_integer_scores(void)
{
  ...
  val   = esl_abc_AvgIScore(a, x, sc); /* average of -1,-6,6,1 = 0 */
  if (val != 0) abort();

  x     = esl_abc_DigitizeSymbol(a, 'M');     /* M = A/C */
  val   = esl_abc_ExpectIScore(a, x, sc, p);  /* expectation of -1,-6 given p = 0.4,0.1 = -2 */
  if (val != -2) abort();
...
degeneracy_float_scores(void)
{
  ...
  x     = esl_abc_DigitizeSymbol(a, 'N'); /* any: A/C/G/T */
  val   = esl_abc_AvgFScore(a, x, sc); /* average of -1,-6,6,1 = 0 */
  if (fabs(val - 0.) > 0.0001) abort();

  x     = esl_abc_DigitizeSymbol(a, 'M');     /* M = A/C */
  val   = esl_abc_ExpectFScore(a, x, sc, p);  /* expectation of -1,-6 given p = 0.4,0.1 = -2 */
...
degeneracy_double_scores(void)
{
  ...
  x     = esl_abc_DigitizeSymbol(a, 'N'); /* any: A/C/G/T */
  val   = esl_abc_AvgDScore(a, x, sc); /* average of -1,-6,6,1 = 0 */
  if (fabs(val - 0.) > 0.0001) abort();

  x     = esl_abc_DigitizeSymbol(a, 'M');     /* M = A/C */
  val   = esl_abc_ExpectDScore(a, x, sc, p);  /* expectation of -1,-6 given p = 0.4,0.1 = -2 */
  if (fabs(val + 2.) > 0.0001) abort();
