# esl_mixdchlet - Mixture Dirichlet distributions

`esl_mixdchlet_Fit()` infers a maximum likelihood mixture Dirichlet
distribution for a data set of count vectors. It uses conjugate
gradient descent from an initial starting point. The result is only a
local optimum, so we typically run it multiple times with different
starting points. The partial derivatives of the log likelihood
function are persnickety, and the purpose of these notes is to enshrine
the derivation that corresponds to the implementation.
...
$$
  L = \log P(\mathrm{data} \mid \theta) = \sum_i \log P(c_i \mid \theta) = \sum_i \log \sum_k q_k P(c_i \mid \alpha_k)
$$

`esl_mixdchlet_logpdf_c()` calculates $\log P(c_i \mid \theta)$.
...
`esl_dirichlet_logpdf_c()` calculates $\log P(c_i \mid \alpha_k)$.
...
$k$ $P(k \mid \theta, c_i)$, calculated by `mixdchlet_postq()`, and its
prior $q_k$.
...
maximizer. The implementation provides the negative log likelihood and
the negative gradient to the CG routine.
